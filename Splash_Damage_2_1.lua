...
function track_wpns()
  ...
  for wpn_id_, wpnData in pairs(tracked_weapons) do
    if wpnData.wpn:isExist() then -- just update speed, position and direction.
      local weaponPos = wpnData.wpn:getPosition()
      wpnData.pos = weaponPos.p
      wpnData.dir = weaponPos.x
...
function onWpnEvent(event)
  if event.id == world.event.S_EVENT_SHOT then
    if event.weapon then
      ...
      local ordnanceType = ordnance:getTypeName()
      if string.find(ordnanceType, "weapons.shells") then
        debugMsg("event shot, but not tracking: " .. ordnanceType)
        return --we wont track these types of weapons, so exit here
      end

      if explTable[ordnanceType] then
        --trigger.action.outText(ordnanceType.." found.", 10)
      else
        env.info(ordnanceType .. " missing from Splash Damage script")
        if splash_damage_options.weapon_missing_message == true then
          trigger.action.outText(ordnanceType .. " missing from Splash Damage script", 10)
...
function modelUnitDamage(units)
  ...
  do
    ...
    if unit:isExist() then --if units are not already dead
      local health = (unit:getLife() / unitDesc.life) * 100
      ...
      if unitDesc.category == Unit.Category.GROUND_UNIT == true and unit:hasAttribute("Infantry") == false and health > 0 then --if ground unit but not infantry
...
function blastWave(_point, _radius, weapon, power)
  ...
  local ifFound = function(obj, val)
    ...
    if desc.category == Unit.Category.GROUND_UNIT then --if ground unit
    ...
    if splash_damage_options.wave_explosions == true then
      ...
      if obj:isExist() then
        if tableHasKey(desc, "box") then
          local length = (desc.box.max.x + math.abs(desc.box.min.x))
          local height = (desc.box.max.y + math.abs(desc.box.min.y))
          local depth = (desc.box.max.z + math.abs(desc.box.min.z))
          ...
          if damage_for_surface > splash_damage_options.cascade_damage_threshold then
            ...
            if obj:getDesc().category == Unit.Category.STRUCTURE then
              explosion_size = intensity *
                  splash_damage_options
                  .static_damage_boost --apply an extra damage boost for static objects. should we factor in surface_area?
